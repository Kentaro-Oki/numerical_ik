...
# Inverse kinematics with joint range constraints
def ik_range(tgt_pose, theta_init):
    _W_E_mat = np.diag(np.array([1, 1, 1, 1, 1, 1]))
    _LINK_LEN_SUM = np.sum(LINK_LEN)
    _damp_param = (_LINK_LEN_SUM**2)*1e-3*np.ones(7)
    _W_N_bar_mat = np.diag(_damp_param)
    ...
    for _ in range(5):
        ...
        _g_vec = np.transpose(_J_mat) @ _W_E_mat @ _e_vec
        theta += (np.linalg.inv(_H_mat) @ _g_vec)[:,0]
        theta = theta*(theta > J_RANGE_MIN) + theta_init*(theta<=J_RANGE_MIN)
        theta = theta*(theta < J_RANGE_MAX) + theta_init*(theta>=J_RANGE_MAX)
...
if __name__ == '__main__':
    score = 0
    time_hist = np.zeros(100)
    theta_init = (J_RANGE_MAX + J_RANGE_MIN)/2
    pose_init = fk(theta_init)
    # Need to prepare some pos/rot samples to evaluate
    theta_tgt = np.random.uniform(low=J_RANGE_MIN, high=J_RANGE_MAX)
    pose_tgt = fk(theta_tgt)
    theta_act = theta_init
    for i in range(100):
        pose_next = pose_init + (pose_tgt - pose_init)*1e-2*i
        start_time = time.perf_counter()
        theta_est_range = ik_range(pose_next, theta_act)
        goal_time = time.perf_counter()
        ik_pose_range = fk(theta_est_range)
        error = pose_next - ik_pose_range
        theta_act = theta_est_range
        time_hist[i] = (goal_time - start_time)*1e3
        print('===========================')
        print('Test No.', i+1)
        print('Actual theta:', np.degrees(theta_act))
        print('Next pose:', pose_next)
        ...
        if (np.linalg.norm(error[:3]) < POS_EPSILON*1e1) and (np.linalg.norm(error[3:]) < ROT_EPSILON*1e1):
            print(colored('OK', 'green'))
            score += 1
        else:
            print(colored('NG', 'red'))
        print('Time:', time_hist[i])
    print('===========================')
    print('Score:', score)